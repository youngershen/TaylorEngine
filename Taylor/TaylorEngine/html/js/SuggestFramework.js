...
String.prototype.encode = function()
{
	var result = "";
	if(this == "") return this;
    // fix the bug for the support for CHINESE and any other Asian Language
    
    if(window.encodeURI){
        
        result = encodeURI(encodeURI(this));
...
{
    if(window.eval && instance.data == "json"){
        
        var jsonObj = window.eval('('+list+')');
        list = jsonObj;
    }
	...
	//fix the bug to support JSON Object 
	for(var i in sfw[instance].suggestions)
	{
		var suggestionContainer = '<tr id="' + sfw[instance].name + '_suggestions[' + i + ']">';
		
		if(sfw[instance].data == "json"){
		    
		     for(var n in sfw[instance].suggestions){
		         
		          for(var m in sfw[instance].suggestions[n]){
		              
		              var value =(String)( sfw[instance].suggestions[n][m].name );
	...
	try
	{
		for(var instance = 0; instance < inputElements.length; instance++)
		{
			   ...
			   getAttributeByName(inputElements[instance], "action"))
			{
				...
				//fix json object method
				sfw[instance].data    = 'normal';
                ...
                // fix the json object support
                if(getAttributeByName(inputElements[instance],"data"))
                    sfw[instance].data = getAttributeByName(inputElements[instance],"data");
